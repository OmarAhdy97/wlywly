      ...
      <div className="dashboard-banner">
        <div className="banner-content">
          ...
          <p className="banner-desc">
            مرحباً بك، لديك <strong style={{ color: '#fed65b' }}>{todaySessions.length} جلسات</strong> اليوم، و <strong style={{ color: '#fed65b' }}>{activeCases.length} قضية متداولة</strong>.
          </p>
        </div>
      ...
      <div className="dashboard-split-grid">
        ...
        <div className="card">
          <div className="card-header">
            ...
            <button className="btn btn-secondary" onClick={() => setActiveTab('agenda')} style={{ fontSize: '0.82rem', padding: '0.35rem 0.75rem', minHeight: 'auto' }}>
              عرض الأجندة
              <ArrowLeft size={14} />
            </button>
          </div>
          ...
          {upcomingCases.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '2.5rem 1rem', color: 'var(--text-muted)' }}>
              <CheckCircle2 size={36} style={{ margin: '0 auto 0.6rem', color: 'var(--status-active)' }} />
              <p style={{ fontSize: '0.9rem' }}>لا توجد جلسات محددة خلال الأيام القادمة.</p>
            </div>
          ) : (
              ...
              <table className="data-table">
                ...
                <tbody>
                  {upcomingCases.map((c) => {
                    ...
                    return (
                      <tr key={c.id}>
                        <td style={{ fontWeight: '700', color: 'var(--primary-700)', whiteSpace: 'nowrap' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
          ...
          <div className="card">
            ...
            {recentRulings.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                {recentRulings.map((r) => (
                  <div key={r.id} style={{ padding: '0.75rem', background: 'var(--bg-card-subtle)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
                      <strong style={{ fontSize: '0.85rem' }}>دعوى {r.case_number}/{r.case_year}</strong>
                      <span className="badge" style={{ background: 'var(--status-judgment-bg)', color: 'var(--status-judgment)', fontSize: '0.7rem' }}>
                        {r.status === 'finalJudgment' ? 'حكم نهائي' : 'قرار جلسة'}
                      </span>
                    </div>
                    <p style={{ fontSize: '0.8rem', color: 'var(--text-main)', lineBreak: 'anywhere' }}>
          ...
          <div className="card" style={{ background: 'linear-gradient(135deg, var(--bg-card), var(--primary-50))' }}>
            <h4 style={{ marginBottom: '0.4rem', fontSize: '0.95rem' }}>طباعة رول الجلسة اليومي</h4>
            ...
            <button className="btn btn-secondary" style={{ width: '100%', fontSize: '0.85rem' }} onClick={() => setActiveTab('agenda')}>
